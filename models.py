...
class CategoryTotal(BaseModel):
    """Sub-model for category total in InsightsInput."""

    total: float = 0.0


class BreakdownTotals(BaseModel):
    """Sub-model for emission categories breakdown in InsightsInput."""

    transport: Optional[CategoryTotal] = None
    energy: Optional[CategoryTotal] = None
    food: Optional[CategoryTotal] = None
    lifestyle: Optional[CategoryTotal] = None


class InsightsInput(BaseModel):
    """Payload for the ``/api/insights`` endpoint, representing carbon calculation results."""

    total_kg_per_year: float = 0.0
    eco_score: int = 0
    breakdown: Optional[BreakdownTotals] = None
