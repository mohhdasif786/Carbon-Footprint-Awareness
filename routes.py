...
from models import ActivityLog, CarbonInput, ChatMessage, GoalInput, InsightsInput
from storage import DataStore

# ─── Public API ───────────────────────────────────────────────────────────────

__all__ = ["router", "store", "limiter"]
...
# ─── Rate Limiter ─────────────────────────────────────────────────────────────
# Shared instance registered in app.py and used by decorators in this module
...
async def serve_index() -> HTMLResponse:
    ...
    if idx.exists():
        try:
            async with aiofiles.open(idx, "r", encoding="utf-8") as fh:
                content = await fh.read()
            return HTMLResponse(content=content)
        except OSError as exc:
            logger.error("Failed to read static/index.html: %s", exc)
            return HTMLResponse(
                f"<h1>Error: Failed to read index.html: {str(exc)}</h1>", status_code=500
            )
    return HTMLResponse(
...
async def get_insights(request: Request, data: InsightsInput) -> Dict:
    """Generate a structured action plan from carbon calculation results.

    Rate-limited to 10 requests/minute per IP.
    """
    try:
        return await get_ai_insights(data.model_dump())
    except Exception as exc:
        logger.error("Insights generation failed: %s", exc)
        raise HTTPException(status_code=500, detail=f"Insights error: {str(exc)}")
...
async def log_activity(activity: ActivityLog) -> Dict:
    ...
    try:
        ...
        logger.error("Activity logging failed: %s", exc)
        raise HTTPException(status_code=500, detail=f"Activity logging error: {str(exc)}")
...
async def set_goal(goal: GoalInput) -> Dict:
    ...
    try:
        ...
        logger.error("Set goal failed: %s", exc)
        raise HTTPException(status_code=500, detail=f"Goal setting error: {str(exc)}")
